...
void MonitoringProcessor::setParam(PARAMETER p, double value)
{
...
void MonitoringProcessor::prepareToPlay(int samplesPerBlockExpected, double sampleRate)
{
	sr = sampleRate;
}

void MonitoringProcessor::getNextAudioBlock(const juce::AudioSourceChannelInfo& bufferToFill)
{
	auto* inL = bufferToFill.buffer->getReadPointer(0);
	auto* inR = bufferToFill.buffer->getReadPointer(1);
	...
	int processing = (int32_t)(A * 16.999);
	...
	double K = tan(juce::MathConstants<double>::pi * biquadL[0]);
	...
	K = tan(juce::MathConstants<double>::pi * biquadR[0]);
...
void MonitoringProcessor::releaseResources()
{
