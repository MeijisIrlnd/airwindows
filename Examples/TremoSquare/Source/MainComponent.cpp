...
MainComponent::MainComponent() : selector(deviceManager, 2, 2, 2, 2, false, false, true, true)
{
    ...
    instantiateSlider(&frequencySlider, &frequencyLabel, TremoSquareProcessor::FREQUENCY, "Frequency", 0.5);
    instantiateSlider(&dryWetSlider, &dryWetLabel, TremoSquareProcessor::DRY_WET, "Dry/Wet", 1);
    addAndMakeVisible(&selector);
    resized();
}

void MainComponent::instantiateSlider(juce::Slider* s, juce::Label* l, TremoSquareProcessor::PARAMETER p, std::string labelText, double initialValue)
{
    ...
    addAndMakeVisible(s);
    s->onValueChange = [this, s, p] { tremoSquare.setParam(p, s->getValue()); };
...
{
    tremoSquare.prepareToPlay(samplesPerBlockExpected, sampleRate);
}

void MainComponent::getNextAudioBlock (const juce::AudioSourceChannelInfo& bufferToFill)
{
    tremoSquare.getNextAudioBlock(bufferToFill);
}

void MainComponent::releaseResources()
{
    tremoSquare.releaseResources();
}
