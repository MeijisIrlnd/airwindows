...
MainComponent::MainComponent() : selector(deviceManager, 2, 2, 2, 2, false, false, true, true)
{
    ...
    instantiateSlider(&cutoffSlider, &cutoffLabel, SidepassProcessor::CUTOFF, "Cutoff", 0);
    addAndMakeVisible(selector);
    resized();
}

void MainComponent::instantiateSlider(juce::Slider* s, juce::Label* l, SidepassProcessor::PARAMETER p, std::string labelText, double initialValue)
{
    ...
    addAndMakeVisible(s);
    s->onValueChange = [this, s, p] {sidepassProcessor.setParam(p, s->getValue()); };
...
{
    sidepassProcessor.prepareToPlay(samplesPerBlockExpected, sampleRate);
}

void MainComponent::getNextAudioBlock (const juce::AudioSourceChannelInfo& bufferToFill)
{
    sidepassProcessor.getNextAudioBlock(bufferToFill);
}

void MainComponent::releaseResources()
{
    sidepassProcessor.releaseResources();
}
