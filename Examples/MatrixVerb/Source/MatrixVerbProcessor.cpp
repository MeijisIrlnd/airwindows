...
MatrixVerbProcessor::MatrixVerbProcessor()
{
	paramBindings = std::map<PARAMETER, float*>{
		{FILTER, &A},
		{DAMPING, &B},
		{SPEED, &C},
		{VIBRATO, &D},
...
void MatrixVerbProcessor::setParam(PARAMETER p, float value)
{
...
void MatrixVerbProcessor::prepareToPlay(int samplesPerBlockExpected, double sampleRate)
{
	sr = sampleRate;
}

void MatrixVerbProcessor::getNextAudioBlock(const juce::AudioSourceChannelInfo& bufferToFill)
{
	...
	biquadC[0] = biquadB[0] = biquadA[0] = ((A * 9000.0) + 1000.0) / sr;
	biquadA[1] = 1.618033988749894848204586;
	biquadB[1] = 0.618033988749894848204586;
	biquadC[1] = 0.5;

	double K = tan(juce::MathConstants<double>::pi * biquadA[0]); //lowpass
	...
	K = tan(juce::MathConstants<double>::pi * biquadA[0]);
	...
	K = tan(juce::MathConstants<double>::pi * biquadC[0]);
	...
	for (auto sample = 0; sample < bufferToFill.numSamples; sample++)
	{
		...
		if (std::fabs(inputSampleL) < 1.18e-37) inputSampleL = fpdL * 1.18e-37;
		if (std::fabs(inputSampleR) < 1.18e-37) inputSampleR = fpdR * 1.18e-37;
		...
		double offsetAL = (std::sin(vibAL) + 1.0) * vibDepth;
		double offsetBL = (std::sin(vibBL) + 1.0) * vibDepth;
		double offsetCL = (std::sin(vibCL) + 1.0) * vibDepth;
		double offsetDL = (std::sin(vibDL) + 1.0) * vibDepth;
		double offsetEL = (std::sin(vibEL) + 1.0) * vibDepth;
		double offsetFL = (std::sin(vibFL) + 1.0) * vibDepth;
		double offsetGL = (std::sin(vibGL) + 1.0) * vibDepth;
		double offsetHL = (std::sin(vibHL) + 1.0) * vibDepth; //L

		double offsetAR = (std::sin(vibAR) + 1.0) * vibDepth;
		double offsetBR = (std::sin(vibBR) + 1.0) * vibDepth;
		double offsetCR = (std::sin(vibCR) + 1.0) * vibDepth;
		double offsetDR = (std::sin(vibDR) + 1.0) * vibDepth;
		double offsetER = (std::sin(vibER) + 1.0) * vibDepth;
		double offsetFR = (std::sin(vibFR) + 1.0) * vibDepth;
		double offsetGR = (std::sin(vibGR) + 1.0) * vibDepth;
		double offsetHR = (std::sin(vibHR) + 1.0) * vibDepth; //R
		...
		double interpolAL = (aAL[workingAL - ((workingAL > delayA) ? delayA + 1 : 0)] * (1 - (offsetAL - std::floor(offsetAL))));
		interpolAL += (aAL[workingAL + 1 - ((workingAL + 1 > delayA) ? delayA + 1 : 0)] * ((offsetAL - std::floor(offsetAL))));

		double interpolBL = (aBL[workingBL - ((workingBL > delayB) ? delayB + 1 : 0)] * (1 - (offsetBL - std::floor(offsetBL))));
		interpolBL += (aBL[workingBL + 1 - ((workingBL + 1 > delayB) ? delayB + 1 : 0)] * ((offsetBL - std::floor(offsetBL))));

		double interpolCL = (aCL[workingCL - ((workingCL > delayC) ? delayC + 1 : 0)] * (1 - (offsetCL - std::floor(offsetCL))));
		interpolCL += (aCL[workingCL + 1 - ((workingCL + 1 > delayC) ? delayC + 1 : 0)] * ((offsetCL - std::floor(offsetCL))));

		double interpolDL = (aDL[workingDL - ((workingDL > delayD) ? delayD + 1 : 0)] * (1 - (offsetDL - std::floor(offsetDL))));
		interpolDL += (aDL[workingDL + 1 - ((workingDL + 1 > delayD) ? delayD + 1 : 0)] * ((offsetDL - std::floor(offsetDL))));

		double interpolEL = (aEL[workingEL - ((workingEL > delayE) ? delayE + 1 : 0)] * (1 - (offsetEL - std::floor(offsetEL))));
		interpolEL += (aEL[workingEL + 1 - ((workingEL + 1 > delayE) ? delayE + 1 : 0)] * ((offsetEL - std::floor(offsetEL))));

		double interpolFL = (aFL[workingFL - ((workingFL > delayF) ? delayF + 1 : 0)] * (1 - (offsetFL - std::floor(offsetFL))));
		interpolFL += (aFL[workingFL + 1 - ((workingFL + 1 > delayF) ? delayF + 1 : 0)] * ((offsetFL - std::floor(offsetFL))));

		double interpolGL = (aGL[workingGL - ((workingGL > delayG) ? delayG + 1 : 0)] * (1 - (offsetGL - std::floor(offsetGL))));
		interpolGL += (aGL[workingGL + 1 - ((workingGL + 1 > delayG) ? delayG + 1 : 0)] * ((offsetGL - std::floor(offsetGL))));

		double interpolHL = (aHL[workingHL - ((workingHL > delayH) ? delayH + 1 : 0)] * (1 - (offsetHL - std::floor(offsetHL))));
		interpolHL += (aHL[workingHL + 1 - ((workingHL + 1 > delayH) ? delayH + 1 : 0)] * ((offsetHL - std::floor(offsetHL))));
		//L

		double interpolAR = (aAR[workingAR - ((workingAR > delayA) ? delayA + 1 : 0)] * (1 - (offsetAR - std::floor(offsetAR))));
		interpolAR += (aAR[workingAR + 1 - ((workingAR + 1 > delayA) ? delayA + 1 : 0)] * ((offsetAR - std::floor(offsetAR))));

		double interpolBR = (aBR[workingBR - ((workingBR > delayB) ? delayB + 1 : 0)] * (1 - (offsetBR - std::floor(offsetBR))));
		interpolBR += (aBR[workingBR + 1 - ((workingBR + 1 > delayB) ? delayB + 1 : 0)] * ((offsetBR - std::floor(offsetBR))));

		double interpolCR = (aCR[workingCR - ((workingCR > delayC) ? delayC + 1 : 0)] * (1 - (offsetCR - std::floor(offsetCR))));
		interpolCR += (aCR[workingCR + 1 - ((workingCR + 1 > delayC) ? delayC + 1 : 0)] * ((offsetCR - std::floor(offsetCR))));

		double interpolDR = (aDR[workingDR - ((workingDR > delayD) ? delayD + 1 : 0)] * (1 - (offsetDR - std::floor(offsetDR))));
		interpolDR += (aDR[workingDR + 1 - ((workingDR + 1 > delayD) ? delayD + 1 : 0)] * ((offsetDR - std::floor(offsetDR))));

		double interpolER = (aER[workingER - ((workingER > delayE) ? delayE + 1 : 0)] * (1 - (offsetER - std::floor(offsetER))));
		interpolER += (aER[workingER + 1 - ((workingER + 1 > delayE) ? delayE + 1 : 0)] * ((offsetER - std::floor(offsetER))));

		double interpolFR = (aFR[workingFR - ((workingFR > delayF) ? delayF + 1 : 0)] * (1 - (offsetFR - std::floor(offsetFR))));
		interpolFR += (aFR[workingFR + 1 - ((workingFR + 1 > delayF) ? delayF + 1 : 0)] * ((offsetFR - std::floor(offsetFR))));

		double interpolGR = (aGR[workingGR - ((workingGR > delayG) ? delayG + 1 : 0)] * (1 - (offsetGR - std::floor(offsetGR))));
		interpolGR += (aGR[workingGR + 1 - ((workingGR + 1 > delayG) ? delayG + 1 : 0)] * ((offsetGR - std::floor(offsetGR))));

		double interpolHR = (aHR[workingHR - ((workingHR > delayH) ? delayH + 1 : 0)] * (1 - (offsetHR - std::floor(offsetHR))));
		interpolHR += (aHR[workingHR + 1 - ((workingHR + 1 > delayH) ? delayH + 1 : 0)] * ((offsetHR - std::floor(offsetHR))));
		//R
		...
		interpolEL = (interpolEL * (1.0 - std::fabs(crossmod))) + (interpolAL * crossmod); //L

		interpolAR = (interpolAR * (1.0 - std::fabs(crossmod))) + (interpolER * crossmod);
		interpolER = (interpolER * (1.0 - std::fabs(crossmod))) + (interpolAR * crossmod); //R
...
void MatrixVerbProcessor::releaseResources()
{
