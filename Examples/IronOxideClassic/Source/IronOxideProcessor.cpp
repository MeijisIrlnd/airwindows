...
IronOxideProcessor::IronOxideProcessor()
{
    paramBindings =
    {
        {INPUT_TRIM, &A},
...
void IronOxideProcessor::setParam(PARAMETER p, float v)
{
...
void IronOxideProcessor::prepareToPlay(int samplesPerBlockExpected, double sampleRate)
{
    sr = sampleRate;
}

void IronOxideProcessor::getNextAudioBlock(const juce::AudioSourceChannelInfo& bufferToFill)
{
	...
	for (auto sample = 0; sample < bufferToFill.numSamples; sample++)
	{
		...
		bridgerectifierL = std::fabs(inputSampleL);
		if (bridgerectifierL > 1.57079633) bridgerectifierL = 1.57079633;
		bridgerectifierL = std::sin(bridgerectifierL);
		...
		bridgerectifierR = std::fabs(inputSampleR);
		if (bridgerectifierR > 1.57079633) bridgerectifierR = 1.57079633;
		bridgerectifierR = std::sin(bridgerectifierR);
		//can use as an output limiter
		if (inputSampleR > 0.0) inputSampleR = bridgerectifierR;
		else inputSampleR = -bridgerectifierR;
		//second stage of overdrive to prevent overs and allow bloody loud extremeness		
		...
		if (std::fabs(drySampleL) > 0.0) {
			inputSampleL += drySampleL;
		}
		if (std::fabs(drySampleR) > 0.0) {
...
void IronOxideProcessor::releaseResources()
{
