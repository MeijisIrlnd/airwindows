...
Biquad2Processor::Biquad2Processor()
{
    paramBindings = {
        {TYPE, &A},
        {FREQ, &B},
...
void Biquad2Processor::setParam(PARAMETER p, double v)
{
...
void Biquad2Processor::prepareToPlay(int samplesPerBlockExpected, double sampleRate)
{
	sr = sampleRate;
}

void Biquad2Processor::getNextAudioBlock(const juce::AudioSourceChannelInfo& bufferToFill)
{
	auto* in1 = bufferToFill.buffer->getReadPointer(0);
	auto* in2 = bufferToFill.buffer->getReadPointer(1);
	auto* out1 = bufferToFill.buffer->getWritePointer(0);
	auto* out2 = bufferToFill.buffer->getWritePointer(1);
	...
	double K = tan(juce::MathConstants<double>::pi * biquad[0]);
	...
	for (auto sample = 0; sample < bufferToFill.numSamples; sample++)
	{
		...
		if (biquad[0] != frequencychase) { biquad[0] = frequencychase; K = tan(juce::MathConstants<double>::pi * biquad[0]); }
...
void Biquad2Processor::releaseResources()
{
